...
def main():
    ...
    # convert threshold specification to list of integers
    try:
        db_search_thresholds = [int(x) for x in args.db_search_thresholds.split()]
    except ValueError:
        sys.stderr.write(f"error: invalid value for --db-search-thresholds: {args.db_search_thresholds}. It must contain a string of spaced numbers, e.g. '50 80 90'")
    try:
        de_novo_thresholds = [int(x) for x in args.de_novo_thresholds.split()]
    except ValueError:
        sys.stderr.write(f"error: invalid value for --de-novo-thresholds: {args.db_search_thresholds}. It must contain a string of spaced numbers, e.g. '50 80 90'")
    try:
        intensity_percentiles = [int(x) for x in args.intensity_percentiles.split()]
    except ValueError:
        sys.stderr.write(f"error: invalid value for --intensity-percentiles: {args.db_search_thresholds}. It must contain a string of spaced numbers, e.g. '50 80 90'")
    try:
        transmission_loss_percentiles = [int(x) for x in args.transmission_loss_percentiles.split()]
    except ValueError:
        sys.stderr.write(f"error: invalid value for --transmission-loss-percentiles: {args.db_search_thresholds}. It must contain a string of spaced numbers, e.g. '50 80 90'")
