...
class GlobalConstants:
    ...
    ncbi_taxonomy_dir: str = (Path.home() / ".metapepview" / "ncbi_taxonomy").as_posix()
    ...
    gtdb_taxonomy_dir: str = (Path.home() / ".metapepview" / "gtdb_taxonomy").as_posix()
    ...
    kegg_map_dir: str = (Path.home() / ".metapepview" / "kegg_datasets").as_posix()
    ...
    de_novo_dropdown_options = [
        ...
        {'label': 'Casanovo', 'value': 'Casanovo'},
    ]

    # local datasets
    reference_dataset_loc: str = resources.files(data) / "share" / "qc_refs"
    crap_fasta_loc: str = resources.files(data) / "share" / "crap.fasta"
    ...
    # spectrum id to scan number patterns
    # for Bruker timsTOF devices, extract the "frame" number
    frame_pattern = re.compile(r"frame=[0-9]+")
    # for Thermo Fisher and several other vendors, extract the "scan" number
    scan_pattern = re.compile(r"scan=[0-9]+")
    # for the Sciex vendor, extract the "cycle" number
    cycle_pattern = re.compile(r"cycle=[0-9]+")
    # If no match found, try to match the "spectrum" number
    spectrum_pattern = re.compile(r"spectrum=[0-9]+")
