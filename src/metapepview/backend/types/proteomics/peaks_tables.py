...
class PeaksDbSearchPsm13(DbSearchMethods):
    """Peaks database search psm table in Peaks Studio 13 format.
    ...
    def get_source_file(cls, file_name: Path | str) -> str:
        ...
        with open(file_name) as contents:
            ...
            source_file_col_idx = header.index("Source File")
            ...
            return Path(line_cells[source_file_col_idx]).stem
...
class PeaksDeNovo13(DeNovoMethods):
    ...
    REQUIRED_FIELDS = ["Source File", "Scan", "Peptide", "Tag length", "ALC (%)",
                       ...
                       "local confidence (%)", "mode", "tag(>=0%)", "Feature Id"]
