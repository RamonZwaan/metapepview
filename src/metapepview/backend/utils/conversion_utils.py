...
def mz_diff_to_ppm(exp_mz: float, meas_mz: float) -> float:
    """Calculate the mass accuracy in ppm from an expected m/z value (based on
    the peptide sequence and charge) and the measured m/z.

    Args:
        exp_mz (float): Expected m/z based on peptide sequence and charge.
        meas_mz (float): Measured m/z in the spectrum.

    Returns:
        float: mass error in ppm
    """
    return (exp_mz - meas_mz) / exp_mz * 10**6
