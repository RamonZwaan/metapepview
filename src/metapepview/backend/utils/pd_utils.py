...
from metapepview.backend.utils.string_utils import digest_proteins, wrangle_peptides
...
def value_to_nan(series: pd.Series,
    ...
    nan_series.loc[series == value] = np.nan
    return nan_series


def regex_over_column(input_col: pd.Series,
                      pattern: str | re.Pattern,
                      no_match_to_nan: bool = True) -> pd.Series:
    """Apply regex pattern conversion using `findall` on a pandas series.
    It takes a series and pattern as input and returns the full converted series
    (in the same shape) as output.

    Args:
        input_col (pd.Series): Input series.
        pattern (str | re.Pattern): regex pattern.
        no_match_to_nan (bool, optional): return nan if no match found, else
            return the original string. Defaults to True.

    Returns:
        pd.Series: _description_
    """
    output_col = deepcopy(input_col)

    regex_series = input_col.str.findall(pattern)
    regex_series = regex_series.apply(
        lambda x: x[0] if len(x) > 0 else np.nan
    )

    # update all rows, nan if no match
    if no_match_to_nan is True:
        output_col = regex_series

    # only update rows with valid regex match
    else:
        match_idx = regex_series.notna()
        output_col.loc[match_idx] = regex_series[match_idx].to_list() 
...
def reshape_taxonomy_df_to_denovo(input_df: pd.DataFrame,
    ...
    metagenome_annot = deepcopy(input_df[mg_annot_cols])
    global_annot = deepcopy(input_df[global_lin_ids + \
                            global_lin_names + \
    ...
    global_annot.loc[:, 'Area'] = global_annot['De Novo Area'] 
    global_annot.loc[:, 'PSM Count'] = global_annot['De Novo Match Count']
    ...
    db_search_col = 'DB search taxonomy'
    if global_annot_de_novo_only is False:
        unipept_col = 'Unipept taxonomy (DB search + de novo)'
    else:
        unipept_col = 'Unipept taxonomy (de novo)'

    metagenome_annot.loc[:, 'Sample Name'] = db_search_col
    global_annot.loc[:, 'Sample Name'] = unipept_col
    
    return pd.concat([metagenome_annot, global_annot]), db_search_col, unipept_col
