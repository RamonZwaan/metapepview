...
- The dashboard can be accessed through both Firefox (Gecko browser engine) and any Chromium based browser.
...
The `-p` flag publishes port 8050 from the container to port 8050 in the host pc. This allows us to reach the dashboard server in the container. The dashboard can be accessed by typing in `http://localhost:8050` in the web browser. Note that the dashboard is only accessible from the host PC as long as the port is closed in the firewall.
...
> [!NOTE]
> For efficient memory handling, the dashboard also supports reading of compressed datasets. For speed improvement and lower RAM usage, compress large files (>100 MB) in `.zip` or `.tar.gz` format.
...
- Peaks 13:  DB search spectral match file exported from Peaks Studio 13. The file is usually named `db.psms.csv`.
...
- FragPipe: DB search PSM file exported from FragPipe. The file is usually named `psm.tsv`. Note: Format of the file depends on the used workflow. To get signal intensity quantification, enable IonQuant label free quantification in FragPipe.
...
- FragPipe: DB search PSM file is stored in `psm.tsv`. PSM's are processed by MSFragger and the file is generated by Philosopher. Additional information may be supplemented from other tools. Most notably, signal intensities from IonQuant.
...
- Peaks 13:  *de novo* spectral identification file exported from Peaks Studio 13. The file is usually named `xxx.denovo.csv`, where `xxx` corresponds to the experiment name. 
...
- Casanovo: *de novo* spectral identification file exported from Casanovo. The results file is usually names `xxx.mztab`, where `xxx` corresponds to the file name specified when configuring Casanovo.
